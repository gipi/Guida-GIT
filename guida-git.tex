...
\minipage
\figuratikz[every path/.style={line width=1pt},show background rectangle
  ]
  \node[commit] (0,0) (tip){c5};
  \node[commit] [above right of=tip] (index){index};
  \node[commit] [below right of=tip]  (wt){wt};
  \node[commit] [left of=tip](c4) {c4};
  \node[commit] [left of=c4](c3) {c3};
  \node[commit] [left of=c3](c2) {c2};
  \draw[->] (tip) -- (c4);
  \draw[->] (c4) -- (c3);
  \draw[->] (c3) -- (c2);
  \node[head] at (tip.north west) (master) {master};
  \node[HEAD] [above=of master](HEAD){HEAD};
  \draw[<-] (master)--(HEAD);
\endfiguratikz[bella]
porcodio
\figuratikz[every path/.style={line width=1pt},
	show background rectangle
  ]
  \node[commit] (0,0) (tip){c5};
  \node[wt] [below right of=tip]  (wt){wt};
  \node[index] [left=of wt] (index){idx};
  \node[commit] [left=of tip](c4) {c4};
  \node[commit] [left of=c4](c3) {c3};
  \node[commit] [left of=c3](c2) {c2};
  \draw[->] (tip) -- (c4);
  ...
  \draw[->] (c3) -- (c2);
  \node[head] at (tip.north west) (master) {master};
  \node[HEAD] [above=of c4] (HEAD){HEAD};
  \draw[<-] (c4)--(HEAD);
\endfiguratikz[bella]
